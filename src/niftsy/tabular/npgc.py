...
class NPGC_special:
    ...
    def _learn_distributions_and_correlation(self, df: pd.DataFrame, epsilon: float | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "transformed_data": z_df,  # Store the Z-space data for potential debugging/analysis
        }
        ...
        if is_int:
            ...
            if epsilon is not None and epsilon > 0:
                ...
                if epsilon is not None and epsilon > 0 and epsilon < 0.01:
                    LOGGER.debug(f"NPGC DEBUG: Raw counts sum: {counts.sum()}")
                    LOGGER.debug(f"NPGC DEBUG: Noisy counts sum: {noisy_counts.sum()}")
            else:
